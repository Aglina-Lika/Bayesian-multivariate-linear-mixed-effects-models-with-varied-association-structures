...
alphaV = 1.63 # association parameter - value
...
for (i in 1:200){
  ...
  id <- rep(1:n, each = K)
  eta.y1 <- as.vector(X1 %*% betas1 + rowSums(Z1 * b1[id, 1:2])) # underlying value but, at the times of y1 
  ...
  eta.y2 <- as.vector(X2 %*% betas2 + rowSums(Z2 * b2[id, 1:2])) + alphaV * eta.y1_m
  ...
  fit <- stan(file = "Value//STAN codes//funcForm_v_assoc_D1D2_no_assoc_D1D2.stan", 
              data = Data, chains = 4, iter = 2000, thin = 1, 
              control = list(max_treedepth = 20, adapt_delta = 0.99))
  # save the time of the STAN model of the i-th simulation 
  time_ela[i]<-sum(get_elapsed_time(fit))/(60*60)  # 1.438005
  ...
  save(summary1, file = paste0("Value//V_assoc_D1D2_no_assoc_D1D2//summary_", i, ".RData"))
}
