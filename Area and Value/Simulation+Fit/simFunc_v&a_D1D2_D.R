...
for (i in 191:200){
  ...
  set.seed(i+1234)
    ...
    fit <- stan(file = "V://Users//055609(A_Lika)//new codes 2 stat paper//linear time _ separated//value+area (time+sex)//funcForm_v&a_D.stan", 
    ...
    summary1 = as.data.frame(summary(fit, 
              pars = c("temp_alphaV","temp_alphaA","temp_betas1","temp_betas2","sigma1","sigma2","D"))$summary)
   
    save(summary1, file = paste0("Area and Value//V_A_D1D2_D//summary_", i, ".RData"))
}
