...
alphaV = 1.63; alphaA = 0.9 # association parameters - value; area

# variance-covariance matrix for the random effects of the 1st linear mixed-effect model
D1 = diag(1, nrow=2, ncol=2) 
# variance-covariance matrix for the random effects of the 2nd linear mixed-effect model
D2 = diag(1, nrow=2, ncol=2) 
...
for (i in 1:200){
  ...
  b1 <- mvrnorm(n, rep(0, 2), Sigma = D1)
  b1 <- as.matrix(b1)
  
  b2 <- mvrnorm(n, rep(0, 2), Sigma = D2)
  b2 <- as.matrix(b2)
  
  
  # Simulate longitudinal responses
  id <- rep(1:n, each = K)
  eta.y1 <- as.vector(X1 %*% betas1 + rowSums(Z1 * b1[id, 1:2])) # underlying value
  
  y1 <- rnorm(n * K, eta.y1, sigma.y)
  
  eta.y1_m <- as.vector(X2 %*% betas1 + rowSums(Z2 * b1[id, 1:2])) # underlying value at the times of the 2nd outcome
  
  
  auc.y1_m <- as.vector(XAderivY2 %*% betas1[1:3] + rowSums(ZAderivY2 * b1[id, 1:2])) # AUC at the times of the 2nd outcome
  
  eta.y2 <- as.vector(X2 %*% betas2 + rowSums(Z2 * b2[id, 1:2])) + alphaV * eta.y1_m + alphaA * auc.y1_m
  ...
  fit <- stan(file = "V://Users//055609(A_Lika)//new codes 2 stat paper//linear time _ separated//value+area (time+sex)//funcForm_v&area_D1D2 .stan", 
  ...
  summary1 = as.data.frame(summary(fit, 
                                   pars = c("temp_alphaV","temp_alphaA","temp_betas1","temp_betas2","sigma1","sigma2","D_1","D_2"))$summary)
...
#time_ela1<-time_ela[!is.na(time_ela)]
#write(time_ela1,"V://Users//055609(A_Lika)//new codes 2 stat paper//linear time _ separated//value+area (time+sex)//simulation results_D1D2_D1D2//time_3_10.text")
