...
for (i in 1:200){
  ...
  set.seed(i+1234)
    ...
    AextraFormY1 <- list(fixed = ~ -1 + year1 + I(year1^2/2) + I(year1 * (sex ==1)),
                         random = ~ -1 + year1 + I(year1^2/2),
                         indFixed = 1:3, indRandom = 1:2)
    
    mfXA_derivY1 <- model.frame(terms(AextraFormY1$fixed), data = DF)
    mfZA_derivY1 <- model.frame(terms(AextraFormY1$random), data = DF)
    XAderivY1 <- model.matrix(AextraFormY1$fixed, mfXA_derivY1)
    ZAderivY1 <- model.matrix(AextraFormY1$random, mfZA_derivY1)
    ...
    auc.y1_m <- as.vector(XAderivY2 %*% betas1[1:3] + rowSums(ZAderivY2 * b[id, 1:2])) # AUC at the times of the 2nd outcome
    
    eta.y2 <- as.vector(X2 %*% betas2 + rowSums(Z2 * b[id, 3:4])) + alphaV * eta.y1_m + alphaA * auc.y1_m
