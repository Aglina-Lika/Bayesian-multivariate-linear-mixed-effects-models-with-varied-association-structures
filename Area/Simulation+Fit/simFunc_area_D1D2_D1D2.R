...
for (i in 191:200){
  ...
  fit <- stan(file = "V://Users//055609(A_Lika)//new codes 2 stat paper//linear time _ separated//area (time +sex)//funcForm_area_cor_uncorD1D2.stan", 
  ...
  summary1 = as.data.frame(summary(fit, 
                                   pars = c("temp_alphaA","temp_betas1","temp_betas2","sigma1","sigma2","D_1","D_2"))$summary)
