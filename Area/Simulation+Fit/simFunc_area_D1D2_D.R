...
# association parameter - area
alphaA = 0.9;
# variance-covariance matrix for the random effects of the 1st and 2nd linear mixed effects model
...
for (i in 191:200){
  ...
  # but this can be easily generalized  # design matrices for the longitudinal measurement model(1 and 2)
  ...
  auc.y1_m <- as.vector(XAderivY2 %*% betas1[1:3] + rowSums(ZAderivY2 * b1[id, 1:2])) # AUC
  
  
  eta.y2 <- as.vector(X2 %*% betas2 + rowSums(Z2 * b2[id, 1:2])) + alphaA * auc.y1_m
  ...
  fit <- stan(file = "V://Users//055609(A_Lika)//new codes 2 stat paper//linear time _ separated//area (time +sex)//funcForm_area.stan", 
  ...
  summary1 = as.data.frame(summary(fit, 
                                   pars = c("temp_alphaA","temp_betas1","temp_betas2","sigma1","sigma2","D"))$summary)
