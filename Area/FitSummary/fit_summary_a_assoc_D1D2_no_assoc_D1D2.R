...
filenames = as.list(dir(path = "Area//A_assoc_D1D2_no_assoc_D1D2", pattern="summary_*"))
...
for(i in 1:200){
  ...
  Fitsummary[i]<-list(get(load(paste0("Area//A_assoc_D1D2_no_assoc_D1D2//", filenames[i]))))
...
df_summary = data.frame(matrix(vector(), 200, 43,
...
for (i in 1:200){
  ...
  df_summary[i,]$mean_D111 = tmp_df[9,]$mean
  df_summary[i,]$n_eff_D111 = tmp_df[9,]$n_eff
  df_summary[i,]$Rhat_D111 = tmp_df[9,]$Rhat
  
  df_summary[i,]$mean_D112 = tmp_df[10,]$mean
  df_summary[i,]$n_eff_D112 = tmp_df[10,]$n_eff
  df_summary[i,]$Rhat_D112 = tmp_df[10,]$Rhat
  
  df_summary[i,]$mean_D122 = tmp_df[12,]$mean
  df_summary[i,]$n_eff_D122 = tmp_df[12,]$n_eff
  df_summary[i,]$Rhat_D122 = tmp_df[12,]$Rhat
  
  df_summary[i,]$mean_D211 = tmp_df[13,]$mean
  df_summary[i,]$n_eff_D211 = tmp_df[13,]$n_eff
  df_summary[i,]$Rhat_D211 = tmp_df[13,]$Rhat
  
  df_summary[i,]$mean_D212 = tmp_df[14,]$mean
  df_summary[i,]$n_eff_D212 = tmp_df[14,]$n_eff
  df_summary[i,]$Rhat_D212 = tmp_df[14,]$Rhat
  
  df_summary[i,]$mean_D222 = tmp_df[16,]$mean
  df_summary[i,]$n_eff_D222 = tmp_df[16,]$n_eff
  df_summary[i,]$Rhat_D222 = tmp_df[16,]$Rhat
